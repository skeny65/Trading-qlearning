...
Estado 3D basado en los indicadores del Pine Script activo:

| Dimension      | Niveles               | Campo en params    | Umbral            |
|----------------|-----------------------|--------------------|-------------------|
| brecha_level   | wide / mid / tight    | `dist_brecha`      | >=0.5% / >=0.2%   |
| pendiente_level| strong / mid / flat   | `pendiente_verde`  | >=0.05% / >=0.02% |
| d300_level     | far / mid / near      | `dist_d300`        | >=1.0% / >=0.3%   |
...
**Ejecucion:** LIVE — ordenes reales en Binance Futures. El `sl` se coloca como STOP_MARKET en Binance.
